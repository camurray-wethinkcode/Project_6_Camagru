...
class VerifyEmail {
	...
	public function check($email) {
		$result = FALSE;
		if (!self::validate($email)) {
			$this->set_error("{$email} incorrect email");
			$this->edebug($this->ErrorInfo);
			if ($this->exceptions) {
				throw new verifyEmailExceptions($this->ErrorInfo);
			}
			return FALSE;
		}
		$this->error_count = 0;
		...
		switch($code) {
		case '250':
			//	echo 'Email provisionally accepted';
			return TRUE;
		case '450':
		//	echo 'Remote mail server does not want to accept mail from your server for some reason...';
			return FALSE;
		case '451':
		//	echo 'Action aborted - local error in processing';
			return FALSE;
		case '452':
		//	echo 'Action not taken - insufficient system storage';
			return FALSE;
		case '550':
			return FALSE;
		default:
	...
	protected function _streamResponse($timed = 0) {
		...
		if (!empty($status['timed out'])) {
			$this->edebug("Timed out while waiting for data! (timeout {$this_stream_timeout} seconds)");
		}
